 ...
 *
 * The status column defaults to NULL so record types without the status field stay empty;
 * new events of a record type that shows the field start as DRAFT instead.
 */
class DataHandlerHook
{
    ...
    {
        if ($status === 'new'
            && $table === 'tx_ximatypo3calendar_domain_model_event'
            && !isset($fieldArray['status'])
            && $this->recordTypeHasStatusField((string)($fieldArray['record_type'] ?? ''))
        ) {
            $fieldArray['status'] = EventStatus::DRAFT->value;
        }
    ...
    private function recordTypeHasStatusField(string $recordType): bool
    {
        $schema = $this->tcaSchemaFactory->get('tx_ximatypo3calendar_domain_model_event');

        return $schema->hasSubSchema($recordType) && $schema->getSubSchema($recordType)->hasField('status');
    }
}
